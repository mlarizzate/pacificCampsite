...
public class CampsitePlaceDeleteRequestValidatorImpl implements CampsiteRequestValidator<Place> {
    @Override
    public List<String> validate(Place place) {
        List<String> errors = new ArrayList<>();

        if(place.getId() == 0) errors.add(Messages.PLACE_DELETE_PLACE_EXPECTED_ID_NOT_FOUND);
